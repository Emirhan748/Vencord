...
export default definePlugin({
    ...
    patches: [
        ...
        {
            // KRİTİK YAMA: Kanal listesi render edilirken VIEW_CHANNEL kontrolünü bypass ediyoruz. 
            // Böylece Discord kanalı gizli algılamayıp orijinal 'channel.name' değerini doğrudan basıyor.
            find: "contentClassName:",
            replacement: [
                {
                    match: /(\i\.\i\.can\([^)]+VIEW_CHANNEL[^)]+\))/g,
                    replace: (m) => `($self.isHiddenChannel(arguments[0]?.channel) || ${m})`
                }
            ]
        }
    ],
    ...
    isHiddenChannel(channel: Channel & { channelId?: string; }, checkConnect = false) {
        try {
            ...
            // NOT: Artık channel.name değerini ezmiyoruz, gelen orijinal adı koruyoruz.
    ...
    resolveGuildChannels(channels: Record<string | number, Array<{ channel: Channel; comparator: number; }> | string | number>, shouldIncludeHidden: boolean) {
        ...
        for (const [key, maybeObjChannels] of Object.entries(channels)) {
            ...
            for (const objChannel of maybeObjChannels) {
                // NOT: Burada da isim değiştirme mantığını tamamen devre dışı bıraktık.
