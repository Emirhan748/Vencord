...
export default definePlugin({
    ...
    patches: [
        ...
        {
            // UI katmanında basılan dinamik "Erişim Yok" (Maskeli) isimleri yakalayıp eklenti ismiyle değiştiren kritik yama
            find: "contentClassName:",
            replacement: [
                {
                    match: /(\i\.\i\.can\(.+?VIEW_CHANNEL.+?\)\?(\i):)/,
                    replace: (_, permissionCheck, realName) => `($self.isHiddenChannel(arguments[0]?.channel) ? "gizli-kanal 🔒" : ${permissionCheck})`
                }
            ]
        }
    ],
